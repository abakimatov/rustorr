...
Ключ считается до снимка, так что правки файлов во время прогона его не
подменяют. Сохраняется только действительный корпус и только если все образы
существовали до снимка (иначе их ID ещё неизвестен). Входы ключа лежат рядом —
