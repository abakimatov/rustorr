...
- **Прокси BT-трафика и публичные адреса — частично** (решение 2026-09-24).
  librqbit 9.0.1 умеет только `socks5://` и один прокси сразу для исходящих
  соединений с пирами и HTTP-трекеров, а публичный адрес в анонсы не
  передаёт. Поэтому `--proxy-mode peers|full` с `socks5://` направляет через
  прокси пиров и HTTP-трекеры (в `peers` трекеры тоже идут через прокси —
  строже, чем у эталона); режим `tracker` (умолчание эталона) и схемы
  `http`, `https`, `socks4`, `socks4a`, `socks5h` — ошибка запуска с
  объяснением. `--public-ipv4/6` проверяются как у эталона и пишутся в лог,
  на движок не влияют.
