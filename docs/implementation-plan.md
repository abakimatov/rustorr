...
Статус: R0–R10 завершены (R10 — 2026-09-25, ветка `r10-rc`, `1.0.0-rc.2`). Этот документ — рабочая точка продолжения между сессиями. Один этап получает статус `in progress` только после явного запуска работ; переход в `done` требует выполнения его критериев выхода и записи доказательств.
...
Статус: `done` (2026-09-24). План и решения — в [`r8-plan.md`](r8-plan.md), доказательства — в [`r8-continuation.md`](r8-continuation.md).
...
Статус: `done` (2026-09-25). План — [`r10-plan.md`](r10-plan.md), доказательства (R1, корпуса, архитектуры, релиз, сеанс на VPS) — [`r10-continuation.md`](r10-continuation.md).
...
- Текущий этап: R10 `done` (2026-09-25, ветка `r10-rc`, `1.0.0-rc.2`);
  R0–R10 завершены. Перед 1.0.0 открыты лицензия и сверка тарифа VPS.
- R10: [`r10-plan.md`](r10-plan.md), [`r10-continuation.md`](r10-continuation.md).
...
- Следующее действие: решение пользователя по лицензии и выпуску 1.0.0;
  слияние веток в `master`; R11 — по запуску пользователем.

### Handoff 2026-09-25 — R10 завершён

- Статус: R10 `done`.
- Выполненная цель: гейты производительности (R1 против свежего MatriX,
  netem закрыт), паритет (5 профилей корпусов), amd64/arm64, воспроизводимые
  бинарники, release candidate, сеанс бюджета на VPS пользователя.
- Изменённые файлы/артефакты: `rustorr-engine` (`resolve_magnet` без
  дубликата соединения; выбор файлов по чтению; лимиты скорости),
  `rustorr-lifecycle` (таймер простоя с добавления; лимиты из настроек),
  `tools/release.sh`, `docker-compose.r10-amd64.yml`, `README.md`,
  `docs/release-notes.md`, `docs/deploy.md`; артефакты — `release/<версия>/`
  (вне git).
- Выполненные команды/тесты: `tools/r4.sh check`, `tools/r4.sh e2e`,
  `tools/r2.sh compare` × 5 профилей, `RUSTORR_R1_TARGET=rustorr
  tools/baseline/r1.sh build|run`, `tools/release.sh build|verify`,
  `tools/deploy.sh <vps> deploy`.
- Доказательства и результаты: `/tmp/rustorr-baseline/r10-*`,
  `/tmp/rustorr-contract/r10-*`, `/tmp/rustorr-contract/r105-*`; таблицы —
  в `r10-continuation.md`.
- Принятые решения: торрент выбирает все файлы до первого чтения, затем
  только прочитанные; `DisableUpload` не применяется (librqbit — только при
  запуске сессии); образы релиза не побайтно воспроизводимы, бинарники —
  да.
- Открытые риски/вопросы: редкий 502 HLS-сегмента при перемотке
  переиспользуемого конвейера GStreamer на EOS (один раз в e2e); открытый файл качается целиком (брошенный
  просмотр стоит остатка файла; окно упреждения требует инвалидации кусков,
  ADR 0004) — кандидат R11; лицензия; тариф трафика VPS.
- Точное следующее действие: решение пользователя по лицензии и 1.0.0;
  слияние `r7-search`, `r8-web`, `r9-deploy`, `r10-rc` в `master`.
- Исходный каталог/commit/конфигурация: `/Users/keito/Documents/pets/rustorr`,
  ветка `r10-rc`; VPS `45.146.131.141` (Rustorr на 8095/8096/6881).
...
- Статус: R8 `done`.
- Выполненная цель: новый веб-интерфейс (React, TypeScript, Tailwind),
  вшитый в бинарник: торренты, плеер (нативный и HLS через GStreamer),
  настройки и сервис, поиск, карта кэша; русский и английский.
- Изменённые файлы/артефакты: ветка `r8-web`; `web/`,
  `crates/rustorr-http/src/web_ui.rs` и `build.rs`, стадия сборки UI в
  `Dockerfile`, `docker-compose.r8-e2e.yml`, `tools/r8/Dockerfile.e2e`,
  `tools/r4.sh web-check|web|e2e`; серверная правка: повторное добавление
  сохранённого торрента по хешу — из сохранённой метаинформации.
- Выполненные команды/тесты: `tools/r4.sh check` (370 Rust, 29 UI),
  `tools/r4.sh e2e` (15 сценариев), `tools/r2.sh compare` для `direct`,
  `r7`, `r7-gst`.
- Доказательства и результаты: `/tmp/rustorr-contract/r83-*`, `r86-gst`,
  `web/e2e/results`.
- Принятые решения: плеер — нативный `<video>` + HLS через `/gst`; языки
  RU/EN; UI вшит в бинарник; дизайн — графит и индиго; PWA и отдельная
  работа по доступности вне R8.
- Открытые риски/вопросы: неприменяемые сетевые поля `BTSets`; magnet без
  трекеров без DHT; нестабильные метаданные субтитров из probe.
- Точное следующее действие: запуск R9 пользователем; слияние `r8-web`
  (и `r7-search`) в `master` — по решению пользователя.
- Исходный каталог/commit/конфигурация: `/Users/keito/Documents/pets/rustorr`,
  ветка `r8-web`.
