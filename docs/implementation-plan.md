...
Статус: R7 `done` 2026-09-24; R0–R7 завершены, R8 не начат. Этот документ — рабочая точка продолжения между сессиями. Один этап получает статус `in progress` только после явного запуска работ; переход в `done` требует выполнения его критериев выхода и записи доказательств.
...
- Текущий этап: R7 `done` 2026-09-24 (ветка `r7-search`); R0–R7 завершены,
  R8 ещё не начат.
...
- Следующее действие: R8 — после запуска этапа пользователем.
