...
Статус: `R7 in progress` с 2026-09-23; R0–R6 завершены. Этот документ — рабочая точка продолжения между сессиями. Один этап получает статус `in progress` только после явного запуска работ; переход в `done` требует выполнения его критериев выхода и записи доказательств.
...
Статус: `in progress` с 2026-09-23. Решения пользователя: Telegram-бот
...
- Текущий этап: `R7` in progress с 2026-09-23; R0–R6 завершены.
...
- План R7 записан в [`r7-plan.md`](r7-plan.md); реализация ещё не начата.
- Следующее действие: R7.0 — capability-профиль эталона, герметичные внешние
  сервисы и первый корпус наблюдений по маршрутам R7.
