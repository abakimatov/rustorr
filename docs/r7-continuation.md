...
Статус: `done` 2026-09-24; R7.0–R7.9 выполнены (R7.6 MCP отложен решением пользователя).
...
## Пробелы R7.0 — чем закрыты

- Медиафикстуры: `clip.wav` (R7.7, ffprobe) и `movie.mkv` (R7.8, GStreamer);
  GStreamer у эталона включается отдельной сборкой с `-tags gst`
  (профиль `r7-gst`).
- DLNA и Bonjour: пробы SSDP/mDNS в internal macvlan-сети `discovery`
  (R7.4), host network не понадобилась.
- `r7-root` — в allowlist с этапом R8 (решение пользователя); `r7-stat` —
  принятое постоянное различие (дамп anacrolix не имеет аналога в librqbit).
- Отложенные пробы профиля `direct` к несуществующим путям
  (`deferred-storage`, `-tmdb`, `-gstreamer`, `-ffprobe`, `-webdav`, `-dlna`,
  а также `-search`, `-download`) стали обычными core-сценариями: ответы
  совпадают, записи allowlist удалены при закрытии R7.
...
## R7.9 — оставшиеся флаги

Имена — в стиле Rustorr (алиасы TorrServer — R9), у каждого флага есть
переменная окружения.

- `--listen` (`RUSTORR_LISTEN`) повторяется или принимает список через
  запятую — аналог повторяемого `--ip`: отдельный сокет и сервер axum на
  каждый адрес с общим роутером (`rustorr_http::Listeners`), повторы
  отбрасываются, занятый адрес — ошибка запуска `cannot listen on: <addr>:
  …`. Порт первого адреса идёт в сгенерированные URL, `/ffp` и GStreamer.
  DLNA ищет первый порт от 9080, свободный на всех адресах, и слушает на
  каждом, как `netbind`.
- `--log-file` (`--logpath`): лог сервера вместо stderr, файл от 100 МиБ
  начинается заново. `--access-log-file` (`--weblogpath`): одна строка на
  запрос в формате `log.WebLogger` — ` 2006/01/02 15:04:05 %3d | %12s |
  %-7s %#v <тело>`, IP клиента по правилам gin по умолчанию (все прокси
  доверенные: `X-Forwarded-For`, затем `X-Real-IP`), путь декодирован, запрос
  сырой, тело multipart скрыто. Сверено с эталоном: строки совпадают байт в
  байт, кроме времени.
- `--dont-kill` (`--dontkill`): SIGHUP, SIGINT, SIGTERM и SIGQUIT пишутся в
  лог и игнорируются, остановка — `GET /shutdown`. Без флага сервер теперь,
  как эталон, останавливается и по SIGHUP и SIGQUIT.
- `--torrent-addr HOST:PORT` или `:PORT` (`--torrentaddr`): адрес и порт
  слушателя пиров librqbit, перекрывает `--peer-port`.
- `--proxy-url`/`--proxy-mode`, `--public-ipv4`/`--public-ipv6` — частично,
  см. решение пользователя в `r7-plan.md`: только `socks5://` в режимах
  `peers`/`full` (пиры и HTTP-трекеры через прокси), остальное — ошибка
  запуска; публичные адреса проверяются и пишутся в лог.

Ограничения:
- Время в web-логе — UTC (у эталона — локальная зона процесса; в
  контейнерах обеих целей это UTC).
- В файл лога попадает вывод `tracing`; эталон перенаправляет в файл весь
  stdout/stderr процесса (паника Rust останется в stderr).
- Прокси: нет режима «только трекеры», нет `http`/`socks4`/`socks5h`; UDP-
  трекеры и DHT идут мимо прокси (как и у anacrolix для UDP). Эталон
  без `--pubipv4` ищет свой публичный адрес в интернете — Rustorr нет.

Доказательства: см. «Закрытие R7».

## Закрытие R7

Критерий выхода `r7-plan.md` выполнен 2026-09-24:

- каждый модуль инвентаря реализован или исключён решением пользователя
  (MCP, Telegram-бот, старый UI); матрица возможностей —
  [`r7-capability-matrix.md`](r7-capability-matrix.md);
- allowlist: пробы `direct` к несуществующим путям стали core-сценариями,
  `r7-stat` и `r7-dlna-root-desc-default-name` — принятые постоянные
  различия (этап `none`); остальные записи — R8 (UI, иконки DLNA), R9
  (`POST /shutdown`), post-R10 (MCP);
- `diff.py` сравнивает и порядок ключей JSON (из сырого тела): Go пишет
  поля структур в порядке объявления; так найдено и исправлено расхождение
  в `/gst/settings`;
- сборка: `tools/r4.sh smoke` — образы arm64 и amd64, запуск и перезапуск
  arm64; amd64-образ и вариант `gstreamer` под amd64 (кросс-сборка через
  multiarch) запущены под эмуляцией (`/echo`, `/gst/echo`).

Доказательства: `/tmp/rustorr-contract/r7-close-r7/` — `r7`, 196 случаев,
0 core-различий, 11 принятых различий точно по allowlist;
`r7-close-direct/` — `direct`, 47 случаев, 0 core-различий;
`r7-close-gst/` — `r7-gst`, 36 случаев, 0 различий. `tools/r4.sh check` —
365 тестов.

## Следующая сессия

R7 закрыт. Следующий этап по `implementation-plan.md` — R8 (новый
веб-интерфейс); перед началом нужен запуск этапа пользователем.

Особенности стенда, которые легко забыть:
- `cargo` на хосте нет, только `tools/r4.sh cargo …`; полная проверка —
  `tools/r4.sh check` (сейчас 365 тестов, включая сборку с `gstreamer`).
